...
X_train = np.load("./predata/yancheng_patch_noaug_oversamples/X_stand_pca_train_" + "testRatio" + str(testRatio) + ".npy")
y_train = np.load("./predata/yancheng_patch_noaug_oversamples/y_stand_pca_train_" + "testRatio" + str(testRatio) + ".npy")
X_test = np.load("./predata/yancheng_patch_noaug_oversamples/X_stand_pca_test_" + "testRatio" + str(testRatio) + ".npy")
y_test = np.load("./predata/yancheng_patch_noaug_oversamples/y_stand_pca_test_" + "testRatio" + str(testRatio) + ".npy")
...
model.add(Conv2D(C1, (3, 3), padding = 'same', input_shape = input_shape))#卷积层1
model.add(BatchNormalization())
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size = pool_size)) #池化层1
model.add(Dropout(0.15))

model.add(Conv2D(2*C1, (3, 3)))
model.add(BatchNormalization())
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size = pool_size))
model.add(Dropout(0.15))

#model.add(Conv2D(4*C1, (3, 3)))
#model.add(BatchNormalization())
#model.add(Activation('relu'))
#model.add(MaxPooling2D(pool_size = pool_size))
#model.add(Dropout(0.25))

#model.add(Conv2D(3*C1, (1, 1)))
model.add(Conv2D(2*C1, (3, 3)))
model.add(BatchNormalization())
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size = pool_size))
...
model.add(Flatten())#
model.add(Dense(2*C1,kernel_regularizer = regularizers.l2(0.001)))#全连接层180个神经元
model.add(BatchNormalization())
model.add(Activation('relu'))
...
checkpointer = ModelCheckpoint(filepath = "./HDF5/yancheng_patch_hdf5/checkpoint1_100.hdf5", verbose = 1, save_best_only = True)
...
model.save('./model/yancheng_patch_model/HSI_model_epochs1_100.h5')
...
plt.savefig("./result/yancheng_patch_result/model_accuracy1_100.svg")
plt.show()
...
plt.savefig("./result/yancheng_patch_result/model_loss1_100.svg")
plt.show()
